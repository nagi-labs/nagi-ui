...
Audit baseline: 2026-09-01. This audit records why Dialog and AlertDialog are
verified against two profiles of `nagi/dialog` revision 1.
...
| WAI-ARIA APG Alert and Message Dialogs Pattern | rolling snapshot reviewed 2026-09-01 | the stricter role and required-message profile reserved for AlertDialog |
...
The second profile supplies the shared platform boundary. AlertDialog layers
...
Node tests separately verify the explicit non-modal `show()` branch, Invoker
Command fallback, same-root close lookup, and idempotent model application.
This separation prevents a modal browser fixture from being presented as proof
of the non-modal branch.

The AlertDialog profile runs the same contract against package and owned DOM.
