...
| [Checkbox](https://base-ui.com/react/components/checkbox) | Checked/unchecked/indeterminate state with form semantics | No Nagi equivalent | **Candidate** | Own a native `<input type="checkbox">`; mirror `indeterminate` only where the DOM property requires JS. Preserve name/value/form/reset/validation. |
...
| [Combobox](https://base-ui.com/react/components/combobox) | Restricted selection, filtering, form integration, multiple values, groups, async/creatable/virtualized variants | Single-select `Combobox` ships filtering, controlled input/selection, disabled-option skip and lossless Escape | **Strengthen** | First add disabled/read-only/required, selected-key form submission, clear, empty and loading. Treat multiple/chips, creatable and virtualization as separate later slices or Blueprints. |
...
| [Field](https://base-ui.com/react/components/field) | Label, control, description and error relationships | Relationship verifier exists, but no field helper or Blueprint | **Candidate** | Prefer `useField()` returning native id/ARIA props plus simple Input-oriented Blueprints. Avoid a compound Field family and behavior wired through slots. |
| [Fieldset](https://base-ui.com/react/components/fieldset) | Grouped controls with legend and disabled grouping | No Nagi equivalent | **Candidate** | Very thin native `<fieldset>` + `<legend>` Blueprint with default content slot. Browser owns group disabling and semantics. |
...
| [Input](https://base-ui.com/react/components/input) | Styled native input foundation | No Nagi equivalent | **Candidate** | Ship a native input Blueprint with attribute fallthrough, `v-model`, invalid/disabled/read-only states and no replacement input state machine. |
...
| [Meter](https://base-ui.com/react/components/meter) | Scalar value within a known range | No Nagi equivalent | **Candidate** | Style native `<meter>` and expose value/min/max/low/high/optimum. No JS measurement state. |
...
| [Progress](https://base-ui.com/react/components/progress) | Determinate/indeterminate task progress | No Nagi equivalent | **Candidate** | Style native `<progress>` and retain a textual label. Browser owns value semantics. |
| [Radio](https://base-ui.com/react/components/radio) | Exclusive checked state with form semantics | No Nagi equivalent | **Candidate** | Own native radio inputs, preferably in a fieldset. An items schema is acceptable for homogeneous choices; preserve form/reset behavior. |
...
| [Select](https://base-ui.com/react/components/select) | Choosing one predefined value from a popup list with form/a11y behavior | CHARTER fixes native `<select>` as the stable path; no package Blueprint yet | **Candidate** | Ship a thin native Select Blueprint. `appearance: base-select` is progressive enhancement; do not rebuild Select from Combobox or depend on `<selectedcontent>` yet. |
...
| [Slider](https://base-ui.com/react/components/slider) | Range selection, orientation and potentially multiple thumbs | No Nagi equivalent | **Candidate** | Ship a native single-thumb `<input type="range">` Blueprint. Multi-thumb collision logic is a separate thick component, not a prop on the native path. |
| [Switch](https://base-ui.com/react/components/switch) | Boolean setting with switch semantics and form behavior | No Nagi equivalent | **Candidate** | Native checkbox with `role="switch"`, label and form behavior. Styling must not hide native focus/disabled/checked semantics. |
...
- `Combobox`: disabled, read-only, required, `name`, selected-key submission,
  clear, empty and loading.
- Add native-first Input, Checkbox, Radio, Switch, Select, Fieldset, Progress,
  Meter and single-thumb Slider Blueprints.
- Add `useField()` only if it materially reduces broken label/description/error
  wiring across those controls.
