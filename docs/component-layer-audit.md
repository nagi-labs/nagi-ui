...
- **Toast is not one component primitive.** `createToastManager` owns data and
  ...
  re-promotion; `useToast` repairs focus through
...
| --------------- | ------------------------------------------------ | ------------------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------- | ---------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Accordion       | `details`, `summary`, native open/name           | controlled key normalization                                  | composed Disclosure surfaces                             | items, summary/panel slots                  | SSR/native-state sync                                            | High        | Each item now composes the public Disclosure Blueprint; Accordion owns only group-level open-key policy.                                   |
...
Review date: 2026-09-04. This pass asks whether a parent Blueprint rebuilds a
child component that could retain its own Contract and Implementation.

| Candidate                         | Decision                  | Contract boundary                                                                                                                          |
| --------------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| Accordion / Disclosure            | composed                  | Disclosure owns each native details/summary interaction; Accordion owns exclusive/multiple open-key coordination.                          |
| DatePicker / DateField / Calendar | next extraction candidate | DateField owns segment editing and Calendar owns grid navigation; DatePicker should own shared value, popup, selection, and focus wiring.  |
| DateRangePicker date surfaces     | next extraction candidate | Two fields and the range calendar retain their child guarantees; the parent owns range transaction and popup integration.                  |
| Autocomplete / Combobox           | API decision required     | Autocomplete is a free-form Combobox policy rather than a structural child. Treating it as a wrapper would erase its Styling Surface.      |
| AlertDialog / Dialog              | shared surface candidate  | AlertDialog has a distinct Contract, but their native dialog renderer may be shared if alert semantics and dismissal policy stay explicit. |
| RangeSlider / Slider              | rejected                  | The two thumbs and shared rail form one coordinated interaction; two independent Slider components would split its state owner.            |
| RangeCalendar / Calendar          | rejected                  | Range preview and start/end selection belong to one grid state owner rather than independent Calendar instances.                           |
| Menubar or ContextMenu / Menu     | rejected                  | Cross-trigger navigation, invocation sessions, and focus restoration require the parent controller to own intermediate state.              |
| MultiSelect / Combobox            | rejected                  | Chips, multiple selection, filtering, and form serialization form a different Contract despite sharing popup mechanisms.                   |

The governing rule is: Contract ownership does not overlap, Implementation
reuse may overlap, and integration evidence may operate through a child without
copying the child's Requirement into the parent. Parent CSS may style a composed
child root for external layout but does not descend into child-owned markup.
Package-managed child components are declared in ownership metadata and shown
by both the CLI and component site. Blueprint-local private children remain
part of the parent's copied ownership bundle.

## Remaining recommended work

Focused implementation work identified by the audit, in rough priority order:

1. extract DateField controls and Calendar surfaces only after their public
   model/event boundary can express DatePicker integration without an
   `embedded` flag or parent access to child DOM internals;
2. extend multi-instance and owner-document tests to each newly reviewed
   collection or popup before calling its implementation publishable;
3. add explicit functional-anatomy failure tests for Carousel, Resizable, Tree,
   Toolbar, Menu, and Toast;
4. unify composite form reset/validity mechanisms only where contracts match;
5. keep the explicit binding-destination audit current as new components gain
   local registration;
6. extend source review evidence when a Behavior boundary changes; the current
