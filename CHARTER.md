...
4. **Asymmetric investment** — Keep areas already handled by native behavior (Dialog/Popover/Tooltip) extremely thin, and invest substantial JavaScript only where there is no native substitute (Combobox/Menu/Listbox and the advanced parts of the adopted date/time family).
...
3. **`data-*`**: only for states with no native or ARIA equivalent. Example: `data-active` for visual focus independent of selection in Listbox / Tree. Menu styles its actual focused native item with `:focus`; a Combobox popup follows the APG selection-follows-focus model and can represent its active option with `aria-selected`. Do not layer `data-active` over either higher-level state. List every used `data-*` **in documentation as part of the public styling contract**.
...
| Menu / Listbox / Combobox | **Thick** (the main JavaScript investment of this project) | Typeahead, focus management, and selection models. Menu keeps every item at `tabindex="-1"` and moves DOM focus to the actual rendered button or anchor so trusted native activation remains intact; the container is only the empty/all-skipped fallback. Listbox and Combobox retain their documented `aria-activedescendant` strategies. Delegate the floating surface to popover while implementing only the interaction model ourselves. |
| Calendar / RangeCalendar / DateField / DatePicker / DateRangePicker / TimeField | **Thick shipped family** (native stable path remains) | Keep `input[type="date"]`, `input[type="time"]`, and `input[type="datetime-local"]` as the stable path for simple values. The shipped advanced path owns inline grids, range selection, segmented editing, and popup coordination. It reuses native Popover, ships no Portal or compound parts, delegates arithmetic to `@internationalized/date`, and preserves ISO form/reset channels. Scheduler/event layout and non-Gregorian era editing remain separate follow-up scopes. |
| Autocomplete / MultiSelect / TagsInput / OTPField | **Thick candidate field family** | Preserve four distinct models: free-form text with optional suggestion commit, selected keys, arbitrary tag strings, and one real OTP input. Native form/reset channels remain visible; Combobox is not given modes that collapse these contracts. This family is not shipped until the final eleven-slice release gate passes. |
| Carousel / Resizable / Toolbar / Tree | Medium to thick candidate coordination | Carousel delegates movement to overflow + scroll snap; Resizable injects separator pointer/keyboard behavior; Toolbar roves direct owned controls; Tree independently owns hierarchical expansion, selection, lazy state, and tree keyboard behavior. No compound part family is introduced. This family remains in release validation. |
| ContextMenu / Menubar / NavigationMenu | **Thick candidate navigation family** | ContextMenu reuses Menu with pointer/long-press coordinates and native auto popover. Menubar adds horizontal coordination over Menu. NavigationMenu remains native `nav` and links with optional native-popover panels and never uses menu roles. Explicit transition-table validation and fresh independent audit are release requirements. |
...
Expect most accessibility implementation effort to concentrate in the Menu/Listbox/Combobox and adopted date/time families. Do not design under the misconception that native delegation makes everything thin.
...
- Menu items use managed DOM focus: every rendered item remains `tabindex="-1"`, while Arrow/Home/End/typeahead move focus to the actual button or anchor. The `role="menu"` container is only the fallback when no enabled item exists. This preserves one focus strategy without synthesizing native link activation.
...
- **2026-07-23** Replaced Menu's container `aria-activedescendant` focus with managed focus on the actual `tabindex="-1"` button or anchor. Independent audit showed that reproducing trusted link modifiers from a container required synthetic clicks and a hidden clone, causing ancestor interception and analytics to fire incorrectly. Actual-item focus lets the browser own one real anchor click, router cancellation, `target`, `rel`, and `download`; Menu core retains only arrow/typeahead/submenu coordination and an empty-menu container fallback. Removed Menu `data-active` in favor of `:focus`, revised Phase 2/2.5 contracts, and added a breaking migration note in `docs/phase2-menu.md`.
- **2026-07-23** Withdrew the premature shipped status of the final eleven adopted slices after independent review found overlay-state races, scattered controlled-write rollback, non-native Menu link activation, and invalid flat Tree ownership. Progress returns to 49/60 shipped (31/37 Base UI-aligned). The candidates remain in the repository while ContextMenu, Menubar, and NavigationMenu are rebuilt from explicit transition tables; controlled settlement and public APIs are then re-audited before tests and a fresh independent review. Canonical redesign: `docs/expanded-overlay-state-machines.md`; release gate: `docs/expanded-catalog-completion.md`.
- **2026-07-23** Shipped Calendar, RangeCalendar, DateField, DatePicker, DateRangePicker, and TimeField as six thick, single-SFC components, moving expanded progress to 49/60 = 81.7% and backlog to 11. Models/forms use ISO strings, arithmetic is delegated to `@internationalized/date`, segmented fields cover locale order, 12/24-hour entry, RTL, mobile beforeinput and paste, and pickers reuse native Popover with no Portal or compound component family. Package exports, ownership, SSR, verified bindings, browser keyboard/form/axe coverage, and both Nagi CSS contracts are required by `docs/date-time-components.md`; native date/time inputs remain the simple stable path.
- **2026-07-23** Adopted Calendar, RangeCalendar, DateField, DatePicker, DateRangePicker, and TimeField as six distinct thick candidates, expanding the general-UI scope to 60 slices and the backlog to 17; progress at adoption was 43/60 = 71.7%. Native date/time inputs remain the stable path for simple values, while the candidates cover advanced inline, range, segmented-field, and popup requirements that native controls do not substitute. React Aria is the focused behavior, accessibility, and internationalization reference for this family. Scheduler and event-calendar products remain separate. Scope and acceptance criteria: `docs/expanded-vue-component-catalog.md` and `docs/date-time-components.md`.
- **2026-07-22** Corrected RangeSlider's visual coordinate system. Both native range inputs now retain the same physical `min` / `max`, while dependent effective limits are updated through `aria-valuemax` / `aria-valuemin` and enforced by the existing model bridge. The owned visual track is inset by the native thumb radius, making its selected-segment endpoints coincide with the thumb centers without replacing the real range controls.
