...
const sourceGroups = [
  [
    "sharedSiteComponents",
    (file) =>
      !file.startsWith("site/pages/") &&
      !file.startsWith("site/components/docs/") &&
      !file.startsWith("site/components/previews/"),
  ],
  ["definitionAndDocsComponents", (file) => file.startsWith("site/components/docs/")],
  ["previewFixtures", (file) => file.startsWith("site/components/previews/")],
  ["pages", (file) => file.startsWith("site/pages/")],
];
...
function classCategory({ token, tag, role, surfaces }) {
  ...
  const expected =
    elementClasses.get(tag) ?? (!excludedSelfMap.has(tag) && !tag.includes("-") ? tag : null);
...
for (const file of files) {
  ...
  const surfaces = new Set(deriveAllowedSurfaceRootNames(file, contract.surfaceRootPrefixes));
  ...
  function visit(node) {
    if (node.type === "VElement") {
      ...
      for (const token of classes) {
        const baseToken = classes.find((candidate) => !candidate.startsWith("-"));
        records.push({
          ...element,
          token,
          category: classCategory({ token, tag, role, surfaces }),
          variantBaseCategory:
            token.startsWith("-") && baseToken
              ? classCategory({ token: baseToken, tag, role, surfaces })
              : null,
        });
      }
    }
...
const categoryVocabulary = Object.fromEntries(
  categories.map((category) => [
    ...
    [
      ...new Set(
        records.filter((record) => record.category === category).map((record) => record.token),
      ),
...
const fullyDerivedBaseRecords = baseRecords.filter((record) =>
  fullyDerivedCategories.has(record.category),
);
...
const sourceGroupResults = Object.fromEntries(
  sourceGroups.map(([name, matches]) => {
    const groupFiles = files.map((file) => path.relative(repository, file)).filter(matches);
    const groupFileSet = new Set(groupFiles);
    const groupRecords = records.filter((record) => groupFileSet.has(record.file));
    const groupVariants = groupRecords.filter((record) => record.category === "variant");
    return [
      name,
      {
        vueSfcFiles: groupFiles.length,
        explicitClassTokens: groupRecords.length,
        variantOccurrences: groupVariants.length,
        variantPercentage: percentage(groupVariants.length, groupRecords.length),
        uniqueVariantStems: new Set(groupVariants.map((record) => record.token)).size,
      },
    ];
  }),
);
const variantBaseCategoryCounts = Object.fromEntries(
  categories
    .filter((category) => !["variant", "unclassified"].includes(category))
    .map((category) => [
      category,
      variantRecords.filter((record) => record.variantBaseCategory === category).length,
    ])
    .filter(([, count]) => count > 0),
);
...
const result = {
  ...
  classification: {
    ...
    openEndedVariants: {
      ...
      fileLocalNames: new Set(variantRecords.map((record) => `${record.file}\0${record.token}`))
        .size,
      byBaseIdentityCategory: variantBaseCategoryCounts,
      bySourceGroup: sourceGroupResults,
    },
  ...
  divAndSpan: {
    ...
    byTag: Object.fromEntries(
      ["div", "span"].map((tag) => [
        tag,
        divSpanElements.filter((element) => element.tag === tag).length,
      ]),
    ),
  ...
  unclassified: {
    ...
    occurrences: unclassifiedRecords.map(({ file, line, tag, token }) => ({
      file,
      line,
      tag,
      token,
