...
test("thin behavior SFCs use only the public component overload", () => {
  ...
  for (const [file, [variable, composable, model]] of Object.entries(controls)) {
    const source = fs.readFileSync(
      path.join(repo, "packages/core/blueprints", file),
      "utf8",
    )
    assert.ok(source.includes(`const ${variable} = ${composable}(props, ${model});`))
    assert.ok(!source.includes(`${composable}(props, ${model}, {`))
