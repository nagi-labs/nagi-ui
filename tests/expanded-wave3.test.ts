...
function keyboard(key: string) {
  let prevented = false;
  return {
    event: { key, preventDefault() { prevented = true; }, stopPropagation() {} } as KeyboardEvent,
    prevented: () => prevented,
  };
}
...
test("ContextMenu opens at pointer coordinates and delegates selection to Menu", async () => {
  ...
  const items = [{ key: "copy", label: "Copy" }, { key: "delete", label: "Delete", disabled: true }];
  const context = scope.run(() => useContextMenu({
    items,
  ...
  context.contextTriggerProps.onContextmenu({
    clientX: 42, clientY: 84, preventDefault() { prevented = true; },
  } as unknown as MouseEvent);
  await new Promise((resolve) => setTimeout(resolve, 1));
  assert.equal(prevented, true);
  assert.equal(context.menu.open.value, true);
  assert.equal(context.anchorStyle.value.position, "fixed");
...
test("ContextMenu terminates rejected and externally closed long-press sessions", async () => {
  ...
  const scope = effectScope();
  const rejected = scope.run(() => useContextMenu({
    items: [{ key: "copy", label: "Copy" }],
    getKey: (item) => item.key,
    getTextValue: (item) => item.label,
    open: rejectedOpen,
  ...
  assert.ok(rejected);
  const touch = {
    pointerType: "touch", pointerId: 7, button: 0, clientX: 10, clientY: 20,
    target: null,
  ...
  rejected.contextTriggerProps.onClickCapture({
    preventDefault() { clickPrevented = true; }, stopPropagation() {},
  } as unknown as MouseEvent);
  assert.equal(clickPrevented, false);

  const acceptedOpen = ref(false);
  const accepted = scope.run(() => useContextMenu({
    items: [{ key: "copy", label: "Copy" }],
    getKey: (item) => item.key,
    getTextValue: (item) => item.label,
    open: acceptedOpen,
...
test("ContextMenu native close before touch release cannot reauthorize the long press", async () => {
  ...
  assert.ok(context);
  const touch = {
    pointerType: "touch", pointerId: 17, button: 0, clientX: 10, clientY: 20,
    target: null,
  } as unknown as PointerEvent;
...
test("Menubar roves top-level menus, opens with ArrowDown, and switches menus", async () => {
  ...
  const binding = scope.run(() => useMenubar({
    menus,
...
test("Menubar commits only accepted opens and keeps an operable owner after a rejected close", async () => {
  ...
  const open = computed({
    get: () => source.value,
    set: (next) => { writes.push(next); if (acceptsWrites) source.value = next; },
  });
  ...
  trigger.onClick({
    currentTarget: { ownerDocument: { getElementById: () => null } }, preventDefault() {},
  } as unknown as MouseEvent);
  await nextTick();
  assert.equal(binding.openMenuKey.value, null);
...
test("NavigationMenu stays native navigation while coordinating optional panels", async () => {
  const scope = effectScope();
  const items = ref([
    { key: "docs", children: [{ href: "/guide" }] },
    { key: "about" },
  ]);
  const open = ref(false);
  const navigation = scope.run(() => useNavigationMenu({
    items,
    getKey: (item) => item.key,
...
test("NavigationMenu promotes previews to activation before toggling closed", () => {
  ...
  trigger.onPointerenter({ pointerType: "mouse", currentTarget: { ownerDocument } } as unknown as PointerEvent);
  assert.equal(open.value, true);
  assert.equal(navigation.activeKey.value, "products");
  trigger.onClick({ currentTarget: { ownerDocument }, preventDefault() {} } as unknown as MouseEvent);
  assert.equal(open.value, true);
  navigation.navProps.onPointerleave();
  assert.equal(open.value, true);
  trigger.onClick({ currentTarget: { ownerDocument }, preventDefault() {} } as unknown as MouseEvent);
...
test("NavigationMenu ends hover preview when the pointer enters a direct link", async () => {
  ...
  assert.ok(navigation);
  const triggerElement = { contains: (target: Node) => target === triggerElement } as unknown as HTMLElement;
  const ownerDocument = {
...
test("Tree expands, enters children, selects, collapses, and repairs removed active entries", () => {
  ...
  appleProps.onPointermove({ stopPropagation() { pointerStopped = true; } } as unknown as PointerEvent);
...
test("Tree never moves active ownership from an enabled child to a disabled parent", () => {
  ...
  scope.run(() => {
    ...
    assert.equal(tree.activeKey.value, "child");
    assert.equal(
      tree.treeProps["aria-activedescendant"],
      `${tree.treeProps.id}-item-child`,
    );
  });
  scope.stop();
});
