...
function focusEvent(currentTarget: object = { isConnected: true }, relatedTarget: object | null = null) {
...
test("Blueprint keeps link semantics, interactive content, and behavior mechanism separated", () => {
  ...
  assert.match(source, /<a\s[\s\S]*?:href="href"[\s\S]*?v-bind="mergeNagiProps\(preview\.triggerProps, \$attrs\)"/)
  assert.match(source, /<span class="unit" popover v-bind="preview\.previewProps">/)
  ...
  assert.doesNotMatch(source, /role="tooltip"|popover="hint"|aria-describedby/)
  assert.doesNotMatch(source, /Teleport|provide\(|inject\(|data-state/)
  assert.doesNotMatch(source, /\b(?:watch|watchEffect|onMounted|onBeforeUnmount|document|window)\b/)
