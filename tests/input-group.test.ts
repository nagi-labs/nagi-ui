...
const sourceFile = path.join(
  repo,
  "packages/core/blueprints/input-group/InputGroup.vue",
);

function normalizeSsrHtml(html: string): string {
  return html
    .replace(/\sdata-v-[\da-f]+(?:-s)?/gu, "")
    .replace(/<!--(?:\[-->|\]-->|-->)?/gu, "");
}
...
async function load(): Promise<{ component: Component; close: () => Promise<void> }> {
  const server = await createServer({
    ...
    plugins: [vue()],
    root: repo,
    cacheDir: fs.mkdtempSync(path.join(os.tmpdir(), "nagi-input-group-vite-")),
    logLevel: "silent",
    server: { middlewareMode: true },
    appType: "custom",
  });
  const component = (
    await server.ssrLoadModule(`/@fs${sourceFile}`)
...
test("InputGroup owns the visual frame without duplicating the native input API", async () => {
  ...
  try {
    ...
    assert.match(
      html,
      /^<div class="n-input-group" id="project-url-group" data-owner="application">/,
    );
            ...
            h(
              ...
              { prefix: "ignored", suffix: "ignored" },
              {
                prefix: ({ prefix }: { prefix?: string }) =>
                  h("span", { class: "n-input-group-prefix", "aria-hidden": "true", title: prefix }, "@"),
                default: () => h("input", { class: "n-input-group-control", "aria-label": "Username" }),
                suffix: ({ suffix }: { suffix?: string }) =>
                  h("abbr", { class: "n-input-group-suffix", title: suffix }, "ID"),
                action: () => h("button", { class: "n-input-group-action", type: "button" }, "Generate"),
              },
            ),
        }),
      ),
...
test("InputGroup remains a styling-only anatomy with a bounded slot surface", () => {
  ...
  assert.match(source, /<slot name="prefix" :prefix="prefix">/);
  assert.match(source, /<slot name="suffix" :suffix="suffix">/);
  ...
  for (const match of source.matchAll(/var\((--nagi-[a-z0-9-]+)\)/g)) {
    assert.ok(themeTokens.has(match[1] as string), `InputGroup uses known token ${match[1]}`);
  }
});
