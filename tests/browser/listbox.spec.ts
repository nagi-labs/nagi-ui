...
test("single mode: selection follows keyboard focus and skips disabled options", async ({
  page,
}) => {
  ...
  await expect(page.getByTestId("single-state")).toHaveText("cherry");

  await single.listbox.press("Home");
  await expect(page.getByTestId("single-state")).toHaveText("apple");

  await single.listbox.press("d");
  await expect(page.getByTestId("single-state")).toHaveText("date");
...
test("multiple mode: Space toggles, Shift+Arrow extends, Ctrl+A toggles all", async ({
  page,
}) => {
  const multiple = section(page, "Multiple");

  await expect(multiple.listbox).toHaveAttribute("aria-multiselectable", "true");
  await expect(page.getByTestId("multi-state")).toHaveText("mushrooms");

  await multiple.listbox.press("ArrowDown");
  await expect(page.getByTestId("multi-state")).toHaveText("mushrooms");

  await multiple.listbox.press(" ");
  await expect(page.getByTestId("multi-state")).toHaveText("mushrooms,anchovies");

  await multiple.listbox.press("Shift+ArrowDown");
  await expect(page.getByTestId("multi-state")).toHaveText("mushrooms,anchovies,onions");

  await multiple.listbox.press("ControlOrMeta+a");
  await expect(page.getByTestId("multi-state")).toHaveText(
    "olives,mushrooms,anchovies,onions",
  );
  await multiple.listbox.press("ControlOrMeta+a");
  await expect(page.getByTestId("multi-state")).toHaveText("none");
});

test("filtering hides options without dropping their selection", async ({ page }) => {
  const multiple = section(page, "Multiple");
  const filter = page.getByLabel("Filter toppings");

  await multiple.listbox.getByRole("option", { name: "Olives" }).click();
  await expect(page.getByTestId("multi-state")).toHaveText("mushrooms,olives");
