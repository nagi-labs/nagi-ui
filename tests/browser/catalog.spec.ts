...
test("Table keeps native reading semantics and custom cell content", async ({ page }) => {
  const table = page.getByRole("table", { name: "Catalog users" });
  await expect(table).toBeVisible();
  await expect(table.getByRole("columnheader")).toHaveCount(3);
  await expect(table.getByRole("rowheader", { name: "Ada Lovelace" })).toBeVisible();
  await expect(table.getByRole("cell", { name: "ada@example.com" })).toBeVisible();
