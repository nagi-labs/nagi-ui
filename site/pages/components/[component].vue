...
import { componentDefinition } from "~/data/component-definitions";
...
const blueprintSource = await loadBlueprintSource(entry.value.name);
const behaviorApis = await loadBlueprintBehaviorApis(blueprintSource);
const highlightedBlueprint = useState<TrustedShikiHtml | "">(
  `blueprint-source:${entry.value.slug}`,
  () => "",
);
...
if (import.meta.server && !highlightedBlueprint.value) {
  highlightedBlueprint.value = await highlightRepositorySource(blueprintSource, "vue");
}
