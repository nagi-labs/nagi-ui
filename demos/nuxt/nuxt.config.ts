...
// CHARTER §10 Phase 0 Demo A. This demo renders the *actual* Dropdown blueprint
// (../../blueprints/dropdown/DropdownMenu.vue) inside a real Nuxt app under
...
export default defineNuxtConfig({
  ...
  // The blueprint imports the library by its published name; resolve it to the
  // workspace source so Vite transpiles it (raw .ts, .ts-extension imports).
  alias: {
    "@nagi-labs/nagi-ui": libEntry,
  },
  vite: {
    // The blueprint SFC lives outside the Nuxt root; allow Vite to read it.
    server: {
      fs: {
        allow: [repoRoot],
      },
