...
<template>
  <div class="n-multi-select">
    ...
    <div class="field">
      <span v-for="(item, index) in selectedItems" :key="item.key" class="chip">
    ...
    <div v-bind="select.popupProps" class="popup" popover>
      ...
      <p v-if="select.visibleItems.value.length === 0" class="status" role="status">{{ emptyText }}</p>
    </div>
  </div>
</template>
...
.n-multi-select {
  ...
  > .popup {
    ...
    border: 1px solid var(--nagi-color-border); border-radius: var(--nagi-radius-overlay); background: var(--nagi-color-surface); box-shadow: var(--nagi-shadow-overlay); color: inherit;
    ...
    > .list > .item[data-active] { outline: 2px solid var(--nagi-color-focus-ring); outline-offset: -2px; }
    > .list > .item[aria-disabled="true"] { color: var(--nagi-color-text-disabled); cursor: not-allowed; }
    > .status { margin: 0; color: var(--nagi-color-text-muted); }
  }
}
@media (forced-colors: active) {
  .n-multi-select > .field:focus-within, .n-multi-select > .popup > .list > .item[data-active] { outline: 2px solid Highlight; }
}
</style>
