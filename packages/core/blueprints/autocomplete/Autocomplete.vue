...
<template>
  <div class="n-autocomplete">
    ...
    <input
      ...
      :required="required"
    />
    <div v-bind="behavior.popupProps" class="popup" popover>
      ...
      <p v-if="behavior.visibleItems.value.length === 0" class="status" role="status">{{ emptyText }}</p>
    </div>
  </div>
</template>
...
.n-autocomplete {
  ...
  > .popup {
    ...
    > .status { margin: 0; padding: var(--nagi-space-item); color: var(--nagi-color-text-muted); }
  }
}
