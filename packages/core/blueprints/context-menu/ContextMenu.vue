...
<template>
  <div class="n-context-menu">
    <span :id="`${context.menu.id}-trigger`" class="unit -assistive">{{ label }}</span>
    <span
...
.n-context-menu {
  display: contents;
  > .unit.-assistive { position: absolute; inline-size: 1px; block-size: 1px; clip-path: inset(50%); overflow: hidden; }
  > .unit.-positioner { position: fixed; }
  > .unit.-target { display: block; }
  > .list.-popup {
    min-inline-size: 12rem; margin: 0; padding: var(--nagi-space-surface-inset); border: var(--n-border-width-1) solid var(--nagi-color-border);
...
@media (forced-colors: active) { .n-context-menu > .list.-popup > .item > :is(.link, .button):focus { outline: 2px solid Highlight; } }
</style>
