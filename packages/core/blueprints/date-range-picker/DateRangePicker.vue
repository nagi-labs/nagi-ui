  ...
  <div v-bind="$attrs" class="n-date-range-picker">
      ...
      >
        ...
        <template v-for="segment in picker.startField.segments.value" :key="segment.key">
          <span
            ...
            class="text -segment"
            :class="{ '-literal': segment.type === 'literal', '-placeholder': segment.value === undefined }"
          >{{ segment.text }}</span>
        </template>
    ...
    >
      ...
      <table
        ...
        :aria-describedby="picker.isInvalid.value ? `${picker.startField.fieldProps.id}-error` : undefined"
      >
        <thead class="rowgroup -head"><tr class="row">
  ...
  > .unit.-fields {
    ...
    > .field {
      ...
      > .text.-segment {
        ...
        &.-literal { cursor: default; }
        &.-placeholder { color: var(--nagi-color-text-muted); }
      }
...
@media (forced-colors: active) {
  ...
  .n-date-range-picker > .unit:not(.-fields) > .header > .button:focus-visible,
  .n-date-range-picker > .unit:not(.-fields) > .table > .rowgroup.-dates > .row > .cell > .button.-day:focus-visible {
    outline: 2px solid Highlight;
    outline-offset: 2px;
  }
  .n-date-range-picker > .unit:not(.-fields) > .table > .rowgroup.-dates > .row > .cell[aria-selected="true"] > .button.-day {
