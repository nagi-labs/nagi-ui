 ...
 *
 * A Definition records what a component guarantees. Standard Requirement sets
 * may be resolved when its module is evaluated, but the Blueprint never reads
 * the result at render time. Documentation, ownership, and tests consume the
 * same resolved value instead of restating it three times.
 *
...
/** Resolve adopted standards and component policy into one complete Definition. */
export function defineComponentDefinition(input: ComponentDefinitionInput): ComponentDefinition {
  ...
  for (const adopted of input.adopts ?? []) {
  ...
  function entries(
    ...
    return [...(adoptedBySection.get(section) ?? []), ...(local ?? [])]
  }
  ...
  return {
    ...
    adoptions: (input.adopts ?? []).map((adopted) => adopted.adoption),
  ...
  for (const [index, part] of definition.anatomy.entries()) {
    ...
    if (parent !== undefined && !parts.has(parent)) {
      issues.push({
        ...
        field: part.directChildOf === undefined
          ? `${field}.within`
          : `${field}.directChildOf`,
      })
    }
 ...
 */
export function inspectAnatomy(
  root: Element,
  anatomy: readonly AnatomyPart[],
): AnatomyIssue[] {
