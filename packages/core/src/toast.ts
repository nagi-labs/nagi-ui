...
export function createToastManager(
  ...
  function add(addOptions: ToastAddOptions): ToastId {
    ...
    if (index >= 0) {
      ...
      mutableToasts.value = mutableToasts.value.map((toast) =>
        toast.id === id ? next : toast,
      )
...
export function useToast(options: UseToastOptions = {}): UseToastReturn {
  ...
  if (!instance) {
    throw new Error("useToast must run during Vue setup; use createToastManager outside components")
  }
  const id = options.id ?? useId()
  const ownsManager = options.manager === undefined
  const manager =
    options.manager ?? createToastManager({
