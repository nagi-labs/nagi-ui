...
/**
 * Portable calendar guarantees used by interchangeable component
 * implementations. Revision 1 required a native button for each selectable
 * date, which is a Blueprint implementation choice rather than a shared
 * calendar guarantee.
 */
export const nagiCalendarRequirementsV2 = defineRequirementSet({
  id: "nagi/calendar",
  title: "Nagi Calendar grid requirements",
  version: "2",
  profile: {
  ...
  requirements: [
    ...
    },
    {
      ...
      section: "semantics",
      classification: "conformant",
      text: "Each selectable calendar date exposes button semantics with an accessible date name.",
      referenceIds: ["apg-grid", "apg-datepicker"],
    },
    {
      id: "INT-01",
    ...
    },
    {
      ...
      section: "focus",
      classification: "conformant",
      text: "The calendar has at most one enabled date control with `tabindex=0`; other date controls are removed from the sequential tab order.",
      referenceIds: ["apg-grid", "apg-datepicker"],
    },
  ],
} as const);
